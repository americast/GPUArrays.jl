function matmul_kernel(state, A::AbstractArray{T}, B::AbstractArray{T}, out, Asize, Bsize, outSize, ::Val{TS}, ::Val{TS²}, ::Val{IntTS²}) where {T, TS, TS², IntTS²}
    ...
    for t in UInt32(1):UInt32(numTiles)
        ...
        @inbounds Asub[gpu_sub2ind((TS, TS), (row, col))] = A[(tiledCol - 1) * Asize[1] + globalRow]
        @inbounds Bsub[gpu_sub2ind((TS, TS), (row, col))] = B[(globalCol - 1) * Asize[2] + tiledRow]
        ...
        for k in UInt32(1):UInt32(TS)
            @inbounds acc += Asub[gpu_sub2ind((TS, TS), (row, k))] * Bsub[gpu_sub2ind((TS, TS), (k, col))]
        end
