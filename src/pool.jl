...
function maxpool2d_kernel(state, A::AbstractArray{T}, out, Asize, pool, stride) where T
    ...
    for p in 1:pool
        ...
        out[idx[1] + size(out, 1) * idx[2]] = temp_max
    end
    return
end


function maxpool2d!(a, out, pool, stride)
    gpu_call(maxpool2d_kernel, a, (a, out, UInt32.(size(a)), pool, stride))
